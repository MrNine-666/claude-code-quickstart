...
| **HC-14** | **PS 版本约束（PS5.1 单运行时）**：`installer/windows/Install.ps1` 及其加载的 `windows/core` / `windows/steps` 模块**全部兼容 PS 5.1+**（前置检测内联，winget/PS7 自动安装为非阻塞推荐组件，**不 re-exec 到 pwsh**）；旧 `Bootstrap.ps1` / `Manage.ps1` 入口已删除。`core/Json.ps1` 提供 PS5.1 兼容的 `ConvertFrom-JsonToHashtable` 替代 PS7 `-AsHashtable`；全栈 `.ps1` 统一 UTF-8 BOM，Windows Release `dist/install.ps1` 构建产物必须使用 UTF-8 BOM（`installer/contracts/build.json` 的 `OutputEncoding` 固定为 `utf8BOM`），否则 PS5.1 在 `irm ... | iex` 读取 GitHub Release `application/octet-stream` 资产时会按 ANSI 解码并导致中文/Unicode 乱码；禁止 PS7 专有语法（`-AsHashtable` / `$PSStyle` / 三元 `?:` / `??` / `&&`/`||` 管道链 / `ForEach-Object -Parallel`） |
| **HC-15** | **Release 单文件 / `irm\|iex` 兼容约束**：`dist/*.ps1` 通过 `irm ... \| iex` 执行时没有稳定脚本文件上下文，`$PSScriptRoot` 为空；同时 PS5.1 对无 BOM 的 Release asset 会误按 ANSI 解码，因此禁止把远程入口改回无 BOM UTF-8，若必须消费无 BOM 远程文本才可在调用端显式 UTF-8 解码。进入单文件 artifact 的 Windows 代码，在 `Join-Path` / `Test-Path` / `Get-Content` / dot-source / contracts 或 templates 查找前必须判空；路径不可用时走 inline fallback、环境变量 fallback 或安全跳过。涉及 contracts、templates、单文件构建、远程入口时，必须同时验证 `pwsh -File` 源码模式与 `irm\|iex` Release 模式。 |
