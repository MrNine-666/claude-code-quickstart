...
assert.match(viewport, /const MAX_PANEL_WIDTH = 72;/, 'ToastViewport 面板宽度必须足够容纳 Pi OAuth 操作提示');

// 全部颜色必须取自 theme，不得出现硬编码色值或裸颜色名，否则 light 主题下失配。
assert.doesNotMatch(
	viewport,
	/(?:fg|bg|borderColor|backgroundColor)=\{?['"]#[0-9a-fA-F]{3,8}['"]/,
	'ToastViewport 不得硬编码 16 进制色值，必须走 theme'
);
assert.doesNotMatch(
	viewport,
...
assert.doesNotMatch(
	viewport,
	/return colors\.info/,
	'info 类型不得 return colors.info 作强调色（两主题为 white/black），应用 colors.primary'
);
...
assert.match(
	store,
	/export function getToastSnapshot\(\)[\s\S]*?return entries;/,
	'getToastSnapshot 必须直接返回模块级 entries（稳定引用），不得每次新建数组'
);
assert.doesNotMatch(
	store,
...
assert.match(
	...
	'ToastViewport 必须用 useSyncExternalStore 订阅 store'
);
...
for (const method of ['success', 'error', 'warning', 'info']) {
	assert.match(
		store,
		new RegExp(`${method}: \\(message: string, duration\\?: number\\)`),
		`toast.${method} 必须保持 (message, duration?) 签名`
	);
}
