...
Claude Code Quickstart 的 **6 菜单管理控制台**（工具管理 / 供应商 / 配置文件 / 全局规则 / MCP / Skills），基于 **OpenTUI + Bun**，经 `bun build --compile` 交叉编译为 4 平台单文件可执行产物（`ccq-windows-x64.exe` / `ccq-windows-arm64.exe` / `ccq-darwin-x64` / `ccq-darwin-arm64`），contracts 内嵌进可执行文件。
