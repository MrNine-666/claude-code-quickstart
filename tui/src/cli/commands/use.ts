...
function runCodexUse(name: string): number {
	...
	if (isOfficialLoginKey(name)) {
		try {
			...
			const msg = redactCodexTomlForOutput(error instanceof Error ? error.message : String(error));
